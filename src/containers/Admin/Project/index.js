...
//=== map state ===
function mapStateToProps({ post, project }) {
  return { post, project };
}
class Project extends Component {
  ...
  componentWillMount() {
    this.props.dispatch(actions.project.fetchProject());
    this.setState({ loading: true });
  }

  componentWillReceiveProps(props) {
    let { project } = props;
    if (project.action == 'PROJECT_FETCH') {
      this.setState({
        loading: false,
        list: project.list
      });
    } else if (project.action == 'PROJECT_SAVE') {
      this.setState({ loading: false, form: { title: '', author: '' } });
      this.props.dispatch(actions.project.fetchProject());
    } else if (project.action == 'PROJECT_DELETE') {
  ...
  onEdit(ID) {
    ...
    this.setState({
      editable: true,
      form: Object.assign({}, list[ID], { projectId: ID })
    });
  }

  onDelete(ID) {
    this.setState({ loading: true });
    this.props.dispatch(actions.project.deletePost(ID));
  }
  ...
  onSubmit() {
    ...
    this.props.dispatch(
      actions.project.saveProject(this.state.form, this.state.editable)
    );
  }
            ...
            <Col xs='6'>
              ...
              <Form>
                <FormGroup>
                  <Label>Title</Label>
                  <Input
                    onChange={value => this.onChangeInputs(value, 'title')}
                ...
                <FormGroup>
                  <Label>Author</Label>
                  <Input
                    onChange={value => this.onChangeInputs(value, 'author')}
